...
def build_model(
    ...
    # Prefer a local LingBot checkpoint on Windows so first-run downloads
    # can be handled out-of-band instead of blocking inside hf_hub_download.
    lingbot_pretrained = os.environ.get("WILDDET3D_LINGBOT_DEPTH_MODEL")
    if not lingbot_pretrained:
        local_lingbot_ckpt = (
            Path(__file__).resolve().parent.parent
            / "ckpt"
            / "lingbot_depth_model.pt"
        )
        if local_lingbot_ckpt.exists():
            lingbot_pretrained = str(local_lingbot_ckpt)
    if not lingbot_pretrained:
        lingbot_pretrained = "robbyant/lingbot-depth-postrain-dc-vitl14"
